...
ModelRenderDefinition::ModelRenderDefinition(const nlohmann::json &json): RenderDefinition(json)
{
    ...
    modColor = ColorDefinitionValue(json, "mod_color", Color(-1));
    affectLightmap = BoolDefinitionValue(json, "affect_lightmap", false);
    skin = NumericDefinitionValue<int32_t>(json, "skin", 0);
}
...
Color ModelRenderDefinition::GetModColor(const Actor &actor)
{
    return modColor.Get(actor.params, Color(-1));
}
...
uint32_t ModelRenderDefinition::GetSkin(const Actor &actor)
{
    return skin.Get(actor.params, 0);
}
